...
RUN R -e "install.packages(c('R6', 'httr','plotly,'dplyr','openssl','sp','sf','rgdal','geometa','shiny','DT','shinyWidgets','jsonlite','remotes'), repos='http://cran.r-project.org')"

RUN R -e "remotes::install_github('eblondel/ows4R')"
RUN R -e "remotes::install_github('daattali/shinycssloaders')"

RUN git -C /root/ clone https://github.com/abennici/FaoCapturePop.git.git && echo "OK!"
RUN mkdir -p /srv/shiny/
RUN ln -s /root/ShinySdilabpopup /srv/shiny/FaoCapturePop
